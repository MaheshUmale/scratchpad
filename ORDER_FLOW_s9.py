...
def _run_hvn_aggregation(db, instrument_key: str, start_time: datetime, end_time: datetime, context: str) -> float | None:
...
def calculate_hvn(db, instrument_key: str) -> float | None:
    """
...
try:
    from ORDER_FLOW_ExtarctInstrumentKeysFromUpstox import extract_unique_instrument_keys
except ImportError:
    print("Error: extract_instrument_keys function not found. Please ensure it is defined or imported.", file=sys.stderr)
    sys.exit(1)
    ...
    if 'trade_manager' in locals():
        if trade_manager.positions:
            ...
            for key, pos in trade_manager.positions.items():
                print(f"  {key}: {pos['position']} @ {pos['entry_price']:.2f} | SL: {pos['stop_loss']:.2f} | TP: {pos['take_profit']:.2f}")
            print("---------------------------------------------------\n")
        else:
